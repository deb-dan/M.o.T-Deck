...
| S9 | 🟡 LATER | Goose slice 2: --resume/--name/--fork surface (history persisted+reachable, tab offers no way in); session-store size readout + prune for data/goose | 🟢 MEDIUM | ⚪ Low | ⚪ Low | 🟢 Good | 🟢 2-5 files | Medium | `@status:in-progress` PROMOTED by Debi live 2026-08-29 ('refresh wipes the session, no way to old sessions — shouldn't be the case'); builder dispatched: reattach-on-reload + sessions strip + Aider echo |
...
- **S28** - The seams the remaining half needs, named so the next builder does not have

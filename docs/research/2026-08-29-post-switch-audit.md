...
signalled. Repo at the time: the now-retained read-only Claude checkout; live snapshot:
`~/Library/Application Support/MOT Deck`.

**The incident state, verified at audit time:**

- `motdeck.yaml` `runner.model` (the pin) = `Qwen3.6-27B-Fable-Fus-711-UnHeretic-NM-DAU-NEO-MAX-NEO-Q4_K_S` — a model whose **file is deleted and which has left the registry** (18 rows in `data/models.json`, 14 chat models, no 27B).
...
4. **NEW — pin-first seeding under a live-first doctrine.** v1.5.56 established "the
   runner's answer outranks motdeck.yaml" and implemented it in exactly one arm
   ...
   **propagates**. The seeder's own `MOT_DECK_WIRE_MODEL` override is the ready seam;
   it has no callers.
...
- **Odysseus** — `seed_odysseus_jan.py`, called with **`MOT_DECK_WIRE_MODEL=<new wire>`**
...
curl -s -H 'Authorization: Bearer motdeck-local' 127.0.0.1:6767/v1/models
python3 -c "import json;print(json.load(open('$HOME/Library/Application Support/MOT Deck/vendor/odysseus/data/settings.json'))['default_model'])"
grep -A2 '^model:' ~/.hermes/config.yaml
grep -B1 -A2 'custom_mot_deck__local:' "$HOME/Library/Application Support/MOT Deck/data/goose/ui-home/goose/config/config.yaml"
python3 -c "import json;print(json.load(open('$HOME/Library/Application Support/MOT Deck/data/opencode/xdg/config/opencode/opencode.json'))['model'])"
ps eww $(cat "$HOME/Library/Application Support/MOT Deck/data/goose-ui.pid") | tr ' ' '\n' | grep GOOSE_MODEL
```
...
is (in MOT Deck, or via the API): fix the pin first (edit `motdeck.yaml

...
- **Current retained batch:** working-tree changes after `ea47eb1`. The user explicitly approved retaining and committing this non-RAM batch on 2026-09-08. At the approval checkpoint Git reported 175 changed paths (121 modified and 54 untracked). It is still **uncommitted, unshipped, and not release-accepted** at this document revision. A new complete gate is running; prior staged runs cannot certify the final tree, and native acceptance remains outstanding.
...
The separate installed-runtime audit executed the 24 skipped contracts: 22 passed and two Voicebox contracts failed. Both failures reproduce at exact baseline `ea47eb1` against the same installed pin, so the candidate did not introduce them; that baseline comparison is evidence, not an exemption. The stale tests search only `*mcp*.py` names and therefore miss `backend/mcp_server/server.py`; the installed implementation calls the supported FastMCP `http_app(path="/", transport="http")` API. No product fault is established by these two failures, but neither test is counted as passed. Correcting their discovery and exercising the mounted tool names/transport remain the first validation item in the next U170 batch.
...
2. **Retained review batch, after `ea47eb1`:** C01–C83 above, with C02/C04/C05/C06 and the RAM portion of C03/C09 explicitly reverted. The user approved committing the remaining batch. Commit, ship and final acceptance evidence must be added only after they actually occur.
...
4. **First item after the retained checkpoint:** repair and execute the two stale Voicebox installed-contract tests so source discovery reaches `backend/mcp_server/server.py` and verifies the actual mounted transport/tool names. This is validation debt, not a third-party fork or a new product feature.
...
The open non-RAM work is: trace every Office document-mismatch producer; establish configured versus effective Office AI context authority; support complete large-workbook fallback rather than relying only on a truncated-save refusal; finish the Music and Generate final-apply responsiveness audit; add crash recovery for interrupted Office AI plugin publication; keep unreadable configuration preservation distinct from repair; distinguish bounded timeout/failure receipts from diagnosis of upstream worker failures; and complete clean packaging plus affected native/all-design journeys. U171 separately records the approved panel-size ceiling investigation.

## Exact changed-file inventory

This lists all tracked changes and new source/test files at the report snapshot. It makes test-only edits and documentation visible alongside application changes. The full Git diff remains the authoritative line-level description.

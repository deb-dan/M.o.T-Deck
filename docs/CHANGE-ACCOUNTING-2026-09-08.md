...
- **Committed continuation:** `2ebb3ae..600d915` contains the nine-file Voicebox contract and Office AI plugin process-crash recovery batch described below. It is committed but **unshipped and not native/FAT accepted**. No vendor or RAM implementation changed.
- **Built v1.5.93 candidate:** source commit `0ae3d87` produced a clean FAT candidate in a fresh output directory without replacing the prior `dist/MOT Deck.dmg`. The built and read-only-mounted copies passed deep signatures and all 12,549 seed-owned hashes, and each contained an inventory of 219 nonempty wheelhouse files. A separate check passed complete ZIP CRCs and WHEEL metadata presence for all 219 archives; offline installation and dependency resolution remain untested. This is FAT artifact evidence, not installation, shipment or native-journey acceptance. Exact hashes and limits are in `docs/BUILD-v1.5.93-CANDIDATE.md`.
...
| C76 | Office AI plugin installer validates archive/SDK/layout/GUID before publication; stages all files and restores the previous installation on ordinary publication failure; final stamp follows successful publication. Its executable fixture checks that no receipt can appear at any intermediate asset rename and that publication happens only after the complete asset set exists. | Cause for mixed/half-installed plugin. **Limit at `2ebb3ae`:** abrupt SIGKILL could leave its install lock/staging area; automatic crash recovery was not established. **Forward pointer:** `600d915` resolves the process-crash cause with the durable recovery mechanism and evidence below; power-loss/FAT/native limits remain. |
...
The committed range contains exactly `bridge/contract_tests/test_voicebox_contract.py`, `bridge/contract_tests/test_installers_contract.py`, `bridge/tests/test_office_plugin_publication.py`, new `bridge/tests/test_office_plugin_crash_recovery.py`, `scripts/install_oo_ai_plugin.sh`, new `scripts/oo_plugin_install.py`, and the accounting/roadmap/ledger documents.
...
5. **Built candidate, `600d915` → `0ae3d87`:** version `1.5.93` and the fresh-output builder contract produced a clean FAT candidate without overwriting the old default image. The candidate DMG is 638,587,539 bytes with SHA-256 `121f9d1b1bf98e26861c388c8803f4629752d4a084d2917406234bee35cccfd8`; the preserved old DMG remains 458,482,571 bytes with SHA-256 `25d67bd1cee3ce890c5c285c291e96c91b5bbda26824416c0dfd943e70a00fe0`. Both the output app and read-only-mounted app passed signatures and 12,549 owned-file hashes, and each inventoried 219 wheelhouse files. A separate check passed complete ZIP CRCs and WHEEL metadata presence for all 219 archives; offline installation and dependency resolution remain untested. The pre-build gate passed 565 contracts plus 24 explicit skips, 1,015 Python tests and all 72 JavaScript files. A concurrent user-run `./scripts/ship.sh` later stopped before deployment on a reproducible shared-runtime defect in the Goose journey test harness. The test-only isolation correction committed as `217ce4d` leaves the FAT payload unchanged; its final gate passed 565 contracts plus 24 explicit skips, 1,016 Python tests and all 72 JavaScript files, while the standalone Goose runner passed all 427 checks concurrently. Full build detail: `docs/BUILD-v1.5.93-CANDIDATE.md`.

Each later U170 batch must extend this sequence with its exact base/head commit, retained/reverted changes, gate and installed-journey evidence, and unresolved limits. A batch is called shipped only after its versioned commit and installed release evidence exist; a verified DMG by itself is not shipment, and planned work is never backfilled into a completed batch.

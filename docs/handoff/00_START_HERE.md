# 00 — START HERE: The MOT Deck Project Handoff
...
- **Living references, in order:** `CLAUDE.md` (canonical working memory), `docs/MOT-DECK-INTERNALS.md`
...
- **Ship rule:** all code ships via `./scripts/ship.sh` (the fat app serves a provisioned snapshot,
  not the repo — see MOT-DECK-INTERNALS §3).
...
| **[04_Roadmap.md](04_Roadmap.md)** | M0→M4 with concrete tasks tied to MOT Deck's current mid-M0 state, de-risking prototypes, and the decision list with recommendations. |
...
- **Correction (2026-07-20, verified against root LICENSE + README in a fresh clone): Jan is Apache-2.0, not AGPLv3.** The earlier "AGPL app / MIT SDK" split is retired. Consequence: the "license trap" argument against forking Jan (01 §Why NOT fork, 03 §core reasoning) no longer applies — the no-fork decision stands unchanged on its two other legs (gaps closed upstream; merge debt forever). The only AGPL component left in MOT Deck is SearXNG. Fixed across 01/02/03/05/06.
...
- **Strategy: DECIDED.** Personal tool first; possibly ship later but not today. No Jan fork — you agreed after seeing the research. Unified product, composed architecture: your MOT Deck (Bridge + Swift Mission Control) is the product; Jan is a swappable runner; Odysseus and Hermes are managed components surfaced through your shell.
...
1. **Finish M0** — get the first Hermes ↔ Odysseus ↔ model-endpoint handshake running through the existing motdeck. Don't pivot to M1 before this runs. Details in [04_Roadmap.md](04_Roadmap.md) §M0.
...
2. **Canonical runner endpoint: headless Jan on :6767** (`jan serve --detach`, Bridge-managed, invisible). Desktop Jan (:1337) stays installed as a manual convenience for model settings and occasional chat — it is NOT MOT Deck's endpoint.
...
7. **Skills convention — accepted:** one canonical SKILL.md corpus (e.g. `~/motdeck/skills`) shared across LM Studio (`~/.lmstudio/skills`) and Hermes; borrow structure from Goose's Apache-2.0 **recipes**.

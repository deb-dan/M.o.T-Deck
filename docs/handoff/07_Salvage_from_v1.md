...
**v1 is not a failed separate project — it is this project's own scaffold, one iteration back.** The doc set's "build state: mid-M0, Bridge and Mission Control exist and work" describes exactly the code in the `Harness project` folder: FastAPI Bridge (~136 lines: status/plan/install/start/stop/logs), dark editorial panel with ⌘K, Swift WKWebView shell, compiled `dist/Harness.app`, Hermes installed in `data/hermes-venv`. The planning docs (00–06) were written as its continuation. **The v1 codebase remains the working M0 scaffold; nothing gets rebuilt from zero.**
...
2. **Port bug caught: :7000 conflicts with macOS AirPlay Receiver.** v1's `harness.yaml` used 7860 for exactly this reason; the doc set had reintroduced :7000. **Fixed 2026-07-20: Odysseus is :7860 throughout this doc set.**
...
5. **Shelved, not rejected — gearbox** (v1 §7): policy-routed local/cloud proxy with privacy globs, escalation, daily budget (`policies/routing.yaml` drafted). Irrelevant while the harness is local-only; revive only if cloud keys ever enter.
...
`Harness project/` stays untouched as the working scaffold + v1 archive. Its `CLAUDE.md` describes v1's state and NEXT ACTION (launch `dist/Harness.app`, fix Hermes start-error surfacing) — still the literal next build step, now tracked from this doc set's M0. When work resumes, update THIS folder's docs; treat v1's CLAUDE.md as historical until the repo is re-pointed at the new plan.

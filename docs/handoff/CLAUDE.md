...
- **Remaining M1 slices:** install-time closure (flip an *uninstalled* component → install its deps too, one approval), live per-step progress (currently the POST is synchronous ~90s), the full state machine (degraded/needs-repair), and the Repair verb. Model provisioning (download) is a node too (M2 model browser makes the model dynamic).

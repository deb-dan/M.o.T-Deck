...
**Next browse slices:** (2) register the same stdio Browser MCP into **Hermes** (`hermes mcp add browsermcp --command npx --args @browsermcp/mcp`, or write `mcp_servers` in `~/.hermes/config.yaml` + reload); (3) add the **http jan-browser-mcp** (:8181) once Debi installs it and reports the endpoint — register transport=http/sse+url into both; (4) resolve the single-extension-connection nuance if both components run the browser MCP at once.

...
async def _transcribe_clip(path: str, audio: list) -> str:
    ...
    THE POINT (root-caused 2026-08-13 from sample's 30s-per-render report): mlx-audio's
...
async def _heal_ref_text(entry: dict, audio: list) -> "dict | None":
    ...
    fix would only ever apply to clips pinned after the upgrade, and sample's existing
...
async def voice_entry_ref(req: Request) -> JSONResponse:
    ...
    on disk (⚠️ PENDING FABLE QA: deliberately not confined to data/voices — sample may
    ...
    # ── LENGTH GUARD (2026-08-14, from sample's 7.28 MB / ~30s mp3) ────────────────
    ...
    if path and not ref_text:
        ...
        # sample's 30s-per-render report): mlx-audio's generate_audio, handed a clip
...
async def voice_library_starter() -> JSONResponse:
    ...
    ⚠️ FIXED 2026-08-14 after sample's Mac reported "2 added — 11 failed". Two defects,
    ...
    for spec in _voice.starter_fetch_order(_voice.STARTER_VOICES):
        ...
        try:
            ...
            # point of the class: it tells sample to click again rather than to believe a
            # voice is gone.
            failed.append({"slot": slot, "speaker": spk,

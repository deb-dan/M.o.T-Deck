...
WHY THIS EXISTS (U55, sample's ruling 2026-09-02, verbatim: *"i think we should have an
...
"components stay up" across a ship TRUE, and it is the behaviour sample wants to keep as
...
own stop is env-fenced with `is_ours`). Anything else on the machine — sample's standalone
...
def _sweep(order: Sequence[str], c: dict, payload: dict) -> JSONResponse:
    ...
    # with both processes. If sample opens the stack tomorrow and something is off, this is

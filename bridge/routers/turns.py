...
async def create_turn(req: Request):
    ...
    try:
        ...
        return JSONResponse(status_code=409, content={
            "id": exc.turn_id,
            "error": "a turn is already running for this lane and session; return to it or press Stop",
        })
    ...
    return JSONResponse(status_code=202, content={k: record[k] for k in ("id", "state", "lane", "session")})


@app.get("/api/turns/active")
async def active_turn(lane: str, session: str):
    try:
        return {"turn": await turns.active(lane, session)}
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
async def turn_events(turn_id: str, after: int = 0):
    ...
    if not await turns.metadata(turn_id):
        raise HTTPException(404, "turn not found")

    async def stream():
        async for event in turns.events(turn_id, after):

...
# directly and none of which stat anything) went on offering models whose weights sample
...
def api_models() -> JSONResponse:
    ...
    try:
        ...
        # HIDDEN: a read-only import the user does not want in their lists (sample's
...
def _rebind_opencode(wire: str) -> str:
    ...
    THE GAP THIS CLOSES (audit §1, and sample's words: "very old deleted models"):
...
def _rescan_fanout() -> str:
    ...
    sample's actual question was "isn't there a way to make the different apps scan?" —
...
def _rebind_dependents(new_id: str, restart_hermes: bool, restart_ody: bool) -> str:
    ...
    # and it is the one sample named ("very old deleted models"): its catalog was rewritten
    ...
    # rebuilt the S29 bug in a new lane and waited for sample to find it again.
...
def _do_switch(new_id: str, old_id: str, restart_hermes: bool, restart_ody: bool) -> None:
    ...
    ⚠️ U15 REWROTE THE FAILURE ARM, AND THE LIVE INCIDENT IS WHY. On 2026-08-29 sample
    ...
    try:
        ...
        if r.returncode != 0:
            ...
            if served and served == new_id:
                ...
                # success — it is the path sample's own switch took (a false non-zero
...
async def api_switch_model(req: Request) -> JSONResponse:
    ...
    # divergence between what the list said and what the loader says. sample's
...
async def api_pin_model(req: Request) -> JSONResponse:
    """PIN THE MODEL THAT IS ALREADY SERVING — the affordance sample hit a wall on.
...
# scripts/start_component.sh. A pattern is not an identity, and sample runs standalone

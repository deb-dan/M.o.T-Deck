...
app/main.swift are deliberately not touched: the page is reachable by URL, sample looks at
...
def _free_port(preferred: int) -> int:
    ...
    that somebody is very often sample's standalone goose Desktop, whose backend picks a
...
def _reap_orphan() -> bool:
    ...
    legacy/stale file, sample's Desktop) gets no signal.
    """
...
def is_ours(pid) -> bool:
    ...
    sample runs the STANDALONE goose Desktop app on this same Mac, and it spawns its own
...
def gooseui_page() -> Response:
    ...
    # ⚠️ THE EMPTY STATE COMES FIRST, AND IT IS AN ACTION, NOT AN APOLOGY (sample's ruling,

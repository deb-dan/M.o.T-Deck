...
# ── Audio HF search (T2) ──────────────────────────────────────────────────────
# WHY: the Audio tab shipped with five curated starters and sample asked the obvious
...
#      "ModelDimensions.__init__() unexpected keyword '_name_or_path'" on sample's
...
# Rationale: this is sample's own machine and personal use is unaffected by CC-BY-NC;
...
def audio_license_badge(license_id: object) -> dict:
    ...
    badge ∈ ok | unknown | nc.
      ...
      unknown → amber badge, Get STILL ENABLED. ⚠️ PENDING FABLE QA: this is sample's

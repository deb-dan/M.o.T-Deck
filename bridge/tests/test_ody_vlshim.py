...
ROWS = [{"id": "local-jan", "name": "Local runner", "base_url": "http://127.0.0.1:6767/v1"},
        ...
        {"id": "renamed", "name": "sample's describer",
         "base_url": "http://127.0.0.1:8700" + PATH + "/"}]
...
check("…and a RENAMED row is still ours (we re-spec to the new name)",
      pick([ROWS[0], ROWS[2]], B).get("name") == "sample's describer")
...
def test_driven():
    ...
    state["rows"][0]["name"] = "sample renamed this"
    ...
    check("a RENAMED endpoint is followed, not fought (spec follows the name)",
          out3["spec"].endswith("@sample renamed this") and len(state["posts"]) == 1)
    ...
    state["settings"]["vision_model"] = "sample-picked-this"
    ...
    check("a HAND-SET vision_model is still never touched",
          ...
          and state["settings"]["vision_model"] == "sample-picked-this")

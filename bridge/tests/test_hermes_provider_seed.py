...
def test_a_comment_the_user_added_survives_when_nothing_changes(tmp_path):
    ...
    cfg.write_text("model:\n  provider: custom\n  base_url: %s\n" % BASE)
    _seed(cfg, root)
    cfg.write_text("# sample's note\n" + cfg.read_text())
    _seed(cfg, root)
    assert "# sample's note" in cfg.read_text()
...
def test_a_rename_survives_a_restart_and_the_main_slot_follows_it(tmp_path):
    ...
    _seed(cfg, root)
    data = _cfg(cfg)
    data["custom_providers"][0]["name"] = "sample's rig"
    data["model"]["provider"] = "custom:mot-deck-(local)"     # what the shell wrote
    open(cfg, "w").write(yaml.safe_dump(data, sort_keys=False))
    out = _seed(cfg, root)
    assert _entry(cfg)["name"] == "sample's rig", "the rename was clobbered"
    assert _slugfile(root)["slug"] == "custom:sample's-rig"
    assert _cfg(cfg)["model"]["provider"] == "custom:sample's-rig", (
...
def test_hand_added_models_and_settings_are_preserved(tmp_path):
    ...
    data["custom_providers"][0]["extra_headers"] = {"X-Tenant": "sample"}
    open(cfg, "w").write(yaml.safe_dump(data, sort_keys=False))
    _seed(cfg, root)
    e = _entry(cfg)
    assert e["models"]["my-own-model"] == {"context_length": 4096}
    assert e["api_mode"] == "chat_completions"
    assert e["extra_headers"] == {"X-Tenant": "sample"}
...
def test_a_model_we_seeded_that_left_the_registry_is_dropped(tmp_path):
    """FOUND LIVE: the row still offered a 27B whose file sample had deleted (it was gone

...
def _b(wb):
    ws = wb.active
    ws.cell(1, 1).number_format = "@"                 # sample: "this column is TEXT"
    ws.cell(2, 1, 5)
    ws.cell(2, 1).number_format = '0.00" kg"'         # sample: "these are kilos"
wb_new("f09.xlsx", _b)
...
finding("F-16", LIE,
        ...
        "sheet to Name(2)",
        "Beta(2)" in names,
        f'sheets went ["Alpha","Beta"] → {names} — sample\'s own "Beta" sheet was renamed '
...
# (the editor saved, another apply landed, sample edited it) → apply runs anyway, against a
...
oo.heartbeat("f20.xlsx", True)                # sample has the workbook open and dirty
...
finding("F-07", LIE,
        ...
        f'op_list lists it as an ordinary workbook, so sample can create exactly this '
...
finding("F-27", LIE,
        ...
        f'a hidden column becoming visible is data sample hid coming back on screen.\n'

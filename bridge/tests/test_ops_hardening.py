"""Ops-path hardening (2026-08-21 slice). Four rulings, all pinned here:
  ...
  4. Port clears establish OWNERSHIP before killing; a foreign listener is refused, not
     killed.  [sample's standalone Unsloth on :8888 being killed by our own start]
...
# command line containing "hermes dashboard"/"hermes serve" — which is precisely sample's
...
check("shell: sample's STANDALONE hermes is FOREIGN (U19 — the whole point)",
      not owns("hermes", "/Users/testuser/.hermes/hermes-agent/venv/bin/hermes "
                         "dashboard --port 9119"))
...
check("shell: sample's STANDALONE Unsloth is FOREIGN (the reported collision)",

...
a = voice.tts_argv(dict(MLX, ref_audio="/lib/sample.wav"), "hi", "/tmp/o/out.wav",
                   "/bin/llama-tts", "/venv/bin/python")
check("a pinned clip becomes --ref_audio <path>",
      a[a.index("--ref_audio") + 1] == "/lib/sample.wav")
check("no transcript ⇒ no --ref_text", "--ref_text" not in a)
a = voice.tts_argv(dict(MLX, ref_audio="/lib/sample.wav", ref_text="one two"), "hi",
...
check("ref_text WITHOUT a clip is not emitted (a caption alone is not a voice)",
      "--ref_text" not in a)
a = voice.tts_argv(dict(MLX, voice="serena", ref_audio="/lib/sample.wav"), "hi",
                   "/tmp/o/out.wav", "/bin/llama-tts", "/venv/bin/python")
check("a name and a clip may coexist (the engine ignores what it cannot use)",
      "--voice" in a and "--ref_audio" in a)
g = voice.tts_argv(dict(GGUF, ref_audio="/lib/sample.wav", ref_text="x"), "hi",
                   "/tmp/o/out.wav", "/bin/llama-tts", "/venv/bin/python")
...
kw = vw.render_kwargs("hi", "serena", "/tmp/x/out.wav", "/lib/sample.wav", "one two")
check("render_kwargs passes the clip through", kw["ref_audio"] == "/lib/sample.wav")
...
check("a plain name gets the recorder's suffix",
      voice.sanitize_clip_name("sample", "wav") == "sample.wav")
check("?fmt beats the typed suffix (the recorder knows its container)",
      voice.sanitize_clip_name("sample.mp3", "wav") == "sample.wav")
check("a name's own suffix is used when no fmt is given",
      voice.sanitize_clip_name("sample.flac") == "sample.flac")
...
check("shell/url punctuation is stripped",
      voice.sanitize_clip_name("sa$mple;rm -rf&", "wav") == "samplerm -rf.wav")
check("an empty name is refused", voice.sanitize_clip_name("", "wav") is None)
check("a name with no usable format is refused",
      voice.sanitize_clip_name("sample", "") is None
      and voice.sanitize_clip_name("sample.webm") is None)
...
with tempfile.TemporaryDirectory() as td:
    ...
    (d / "sample.wav").write_bytes(b"RIFF" + b"\0" * 100)
    ...
    check("only audio files are listed, sorted",
          [c["name"] for c in lib] == ["aunt.mp3", "sample.wav"])
    check("a listing carries name, stem, path and size",
          lib[1]["stem"] == "sample" and lib[1]["size"] == 104
          and lib[1]["path"].endswith("sample.wav"))
    check("a subdirectory is not a clip", all(c["name"] != "sub" for c in lib))

    p1 = voice.unique_clip_path(str(d), "sample.wav")
    check("never-clobber: an existing name gets a ' (n)' suffix",
          os.path.basename(p1) == "sample (1).wav")
    (d / "sample (1).wav").write_bytes(b"x")
    check("never-clobber counts up",
          os.path.basename(voice.unique_clip_path(str(d), "sample.wav")) == "sample (2).wav")
    check("a free name is used as-is",
          os.path.basename(voice.unique_clip_path(str(d), "new.wav")) == "new.wav")

    p, why = voice.library_target("sample.wav", R)
    check("library_target resolves a real clip", p and p.endswith("sample.wav") and why is None)

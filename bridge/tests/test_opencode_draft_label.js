 ...
 * root-caused sample's growing pile of OpenCode "New session" tabs: they are not sessions
 ...
 * sample's ruling: leave the landing alone, and stop the drafts calling themselves work
 ...
 *
 * THE DISCRIMINATOR (sample's live bug, 2026-08-29). The first cut relabelled EVERY
...
ok(RAW.indexOf('runner auto session') > 0, "…and the label is sample's words");
...
(async () => {
  ...
  // ── 2. THE DISCRIMINATOR — sample's bug ─────────────────────────────────────
  w = world({ store: [], version: PIN });
  await start(w);

...
THE USER STORY THIS FILE IS THE GATE FOR, in sample's own words (2026-08-29):
...
# sample's actual dead models — used as fixture ids throughout, so a future reader can
...
def test_an_unplugged_disk_does_not_empty_every_picker(tmp_path):
    """★ THE ADVERSARIAL FINDING OF THIS SLICE. sample keeps models in LM Studio's library
...
def test_opencode_is_in_the_fan_out():
    ...
    sample named. Both triggers now reach it: a model switch and a RESCAN."""
    from bridge.appsrc import APP_SOURCE as APP

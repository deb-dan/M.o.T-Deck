...
2. THE FOUR UNMET STATES, each with the ONE action that actually fixes it — including
   ...
   fails. (Measured live on sample's Mac 2026-08-29: config said Qwen3.6-27B…, the runner
...
3. THE TWO SILENCE GATES — a component that is not running says nothing (its tab already
   ...
   (sample's partial-install principle, ledger S23).
...
def test_two_strikes_before_we_accuse_a_disk():
    ...
    evidence. Telling sample her model file is gone when it is merely slow is the same
...
def test_the_pin_affordance_exists_end_to_end():
    """Audit §5.5, the wall sample hit: with pin ≠ served the drift state was STABLE."""
...
#
# THE INCIDENT (sample, screenshot, 2026-09-02, and the reason this is not just more of

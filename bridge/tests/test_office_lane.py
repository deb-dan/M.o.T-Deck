...
# surface sample actually saves through is the embedded editor, whose x2t round-trip
...
# ⚠️ THE COUNT MOVED FROM >=10 TO >=9 AT THE 9+3 RULING (sample 2026-08-29) AND THE STRIP
...
check("the title is ONE word — a two-word title does not fit the strip budget",
      " " not in "LOffice")
check("…and the reason sample's 'Office Lane' was not taken is recorded next to it",
...
# was no container to measure and no 0x0-canvas trap left to guard. Then sample's
...
# These lines used to be logged as `[office] diag …`, and the instruction handed to sample

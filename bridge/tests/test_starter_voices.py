...
What is pinned:
  ...
  * THE 2026-08-14 REGRESSION, reproduced: walking the slot table in accent order while
    ...
    (that was Debi's "2 added — 11 failed"); walking starter_fetch_order() finds all
...
# Debi's Mac that turned one rate-limited request into "speaker not found in the dataset
...
# below one already found. Debi's Mac: 2 added, 11 failed.
BIG = []
...
with tempfile.TemporaryDirectory() as td:
    ...
    check("Debi's actual clip (7.28 MB mp3) estimates as long enough to act on",

...
THE JOURNEYS THAT WERE ACTUALLY WALKED ON THE REAL STACK (2026-08-29, this Mac):
  ...
  J10 NEVER-CLOBBER, LIVE. `display_name` hand-edited to "sample hand-edited this" plus a
...
THE ADVERSARIAL FINDINGS THIS FILE FENCES (all fixed):
  ...
  A2  ⚠️ READ IDENTITY BEFORE KILL. A restart helper cleared "our" ACP port with
      `lsof -ti tcp:N | xargs kill -9` and closed sample's own standalone goose Desktop
...
def test_entry_document():
    ...
    ok(f"<title>{G.SURFACE_NAME}</title>" in out and G.SURFACE_NAME == "Goose UI",
       "OUR surface is named Goose UI (sample 2026-08-29); the bundle is not rebranded")
...
def test_no_kill_by_port_or_pattern():
    ...
    ok(not re.search(r"lsof[^\n]*kill", code),
       ...
       "choosing another, because the process holding it may be sample's own goose")
    import ast
...
def test_named_provider_file_matches_what_goose_itself_wrote():
    ...
    # OLD behaviour — the one that kept goose's picker offering the models sample deleted
...
def test_absent_dependency_lands_on_something_usable():
    ...
    # the button, then progress, then the UI (sample's house-component ruling).
...
def test_the_lane_is_REACHABLE():
    ...
    ok(("gooseui", False) in nav.DEFAULT_TOPBAR,
       ...
       "two lanes sample's ruling says coexist")
    ...
    # THE RENAME (sample's naming ruling 2026-08-29): every place the PTY lane's label is
...
def test_the_sidebar_delete_speaks_gooses_own_protocol():
    """sample asked twice for a per-chat delete in the CHATS list. Upstream has none there

...
3. VALIDATE. Two rules, and both exist so a customisation cannot LOSE something —
   nothing hidden on every bar it is allowed on, and at most 12 pinned tabs (sample's
...
def test_registry():
    ...
    # sample's amendment: the two newest lanes ARE in the registry — that is how they
    ...
    # ⚠️ THE MUSIC PAIR, REWRITTEN AT THE CONSOLIDATION SLICE (sample 2026-08-29) AND NOT
    # WEAKENED. The Compose slice's rule was "both stay until sample chooses"; she chose:
    ...
    # embedded surface at /gooseui/ driving a goosed we supervise. sample's ruling is that
    ...
    # v1.5.60 (sample live): tab_only REVOKED — it let Classic into the window/⋯ menu,
...
def test_normalize():
    ...
    # ⚠️ WAS 11 PINS UNTIL THE 9+3 RULING (sample 2026-08-29), AND THE STRIP STILL DRAWS
    ...
    ok(nav.NAV_TOPBAR_PINS == 9 and nav.NAV_TOPBAR_MRU == 3
       ...
       "9 stable pins + a 3-slot window = sample's 12, unchanged")
    ...
    # lane by default would silently declare a winner between two lanes sample's ruling
    ...
    ok(len(nav.strip(m)) <= nav.NAV_TOPBAR_MAX,
       "…and the strip it derives is never longer than sample's twelve")
    # IDEMPOTENT
...
def test_validate():
    ...
    # THE 9+3 RULING AND sample'S 12 DID NOT MOVE: the strip still holds twelve, three of
    ...
    ok(nav.NAV_TOPBAR_MAX == 12, "the STRIP is still sample's 12")
...
# the SAVED order back. Without the migration sample's reorder is invisible on sample's own
...
def test_reorder_migration():
    ...
    ok([i for i, p in nav.DEFAULT_TOPBAR if p][:5]
       ...
       "sample's order leads with the deck and the three agent/model lanes")
    ...
    ok(moved and [r["id"] for r in got["topbar"]] == new,
       "an UNTOUCHED v1 layout is replaced with sample's order")
...
def test_reorder_migration_on_disk_runs_exactly_once():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ok(nav.visible(m, "topbar") == [i for i, q in nav.DEFAULT_TOPBAR if q],
           "an untouched v1 file on disk reads back as sample's order")
    ...
    with tempfile.TemporaryDirectory() as td:
        ok(nav.visible(nav.read(td), "topbar") == [i for i, q in nav.DEFAULT_TOPBAR if q],
           "a fresh machine gets sample's order with no migration involved")
...
def test_wiring():
    ...
    ok('MOTDeckTab(id: "music"' not in SWIFT
       ...
       "…titled as sample named it, id unmoved")
...
def test_routes_live():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            # ── THE REORDER ROUND TRIP (sample: "reorder not working", 2026-08-21) ──
            ...
            # generation moves, and the file agrees. The bug sample saw was PANEL-side (the
...
# ── 7. THE LAST-THREE WINDOW, as journeys (sample's ruling 2026-08-29) ─────────
...
def test_music_is_one_door():
    """sample's consolidation ruling, executed on a saved layout: ONE Music row and ONE

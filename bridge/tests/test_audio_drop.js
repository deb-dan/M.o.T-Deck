...
// ── tab-count generalization (standing rule, sample 2026-08-20) ──
...
check('the two optional component tabs are rows in that table',
      ...
      // 8899, NOT upstream's 8888: that port belongs to sample's standalone Unsloth app and
...
// ⚠️ TITLED "Music Classic" SINCE THE CONSOLIDATION SLICE (sample 2026-08-29: ONE Music
...
// v1.5.60: the Classic shell tab is GONE (sample hit ⋯ -> a second Music tab).
...
// sample's ask: dragging a tab the OTHER pane already shows opens a SECOND copy here
...
// beside Mission Control was never checked (sample's own layout: toggle a toolset, wait
...
// REVISED (v1.5.50, sample's live feedback): the ruling CHANGED — Studio<->Classic swap
...
// ══ THE LAST-THREE WINDOW ITSELF (sample's ruling 2026-08-29) ══════════════════════
...
// Bar" (sample's affordance for the strip toggle), and a menu that only exists when an
...
// ── v1.5.26: HIDE / SHOW THE NATIVE TAB STRIP (sample) ────────────────────────────
...
// v1.5.26 — sample'S ORDER. The same eleven ids in a new reading order; bridge/nav.py's
...
check('the default strip is still the eleven default tabs, in sample\'s order',

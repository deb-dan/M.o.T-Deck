...
THE USER STORY THIS FILE IS THE GATE FOR: sample opens the Odysseus tab, renames the
...
def test_rename_survives_every_start():
    ch, notes = seed.endpoint_plan(row(name="sample's box"), MARKER, WANT)
...
def test_rename_survives_even_with_no_marker_file():
    ch, _ = seed.endpoint_plan(row(name="sample's box"), {}, WANT)
...
def test_working_or_unverifiable_user_key_survives_but_rejected_managed_key_rotates():
    ch, notes = seed.endpoint_plan(
        row(api_key="sk-users-own"), MARKER,
...
def test_three_starts_in_a_row_after_a_rename_and_a_disable():
    ...
    r, m = row(name="sample's box"), dict(MARKER)
    for _ in range(3):
        ch, _ = seed.endpoint_plan(r, m, WANT)
        r, m = apply(r, ch), seed.next_marker(m, ch)
    assert r["name"] == "sample's box"
    r["is_enabled"] = False
    for _ in range(3):
        ch, _ = seed.endpoint_plan(r, m, WANT)
        r, m = apply(r, ch), seed.next_marker(m, ch)
    assert r["is_enabled"] is False and r["name"] == "sample's box"
...
def test_a_model_whose_file_is_gone_is_never_pinned(tmp_path):
    """sample's incident, as a unit: she deleted the muse/glimmer family in LM Studio and
...
#
# THE INCIDENT, AS A USER STORY: sample switched the runner to Parable-Qwen3-4B and

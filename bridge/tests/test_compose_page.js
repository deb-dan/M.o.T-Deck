...
// ⚠️ REWRITTEN AT THE CONSOLIDATION SLICE (sample 2026-08-29). The Compose slice's rule
// was "two rows, both stay until sample picks". She picked: ONE door, both looks. So the
...
ok(/function musicClassic\(\)/.test(html) && !/cmd:'switchTab'/.test(html)
   ...
   + '(sample live feedback 2026-08-29: it parked a second Music tab in a strip window '
...
ok(/never block|render anyway/i.test(M.tipPlain(M.ramVerdict(MINIMAX))),
   '…and its hover says it is advisory (sample’s advisory-gates ruling)');
...
//     This is the screen sample looks at, and it carries ZERO sentences.
...
// text elements. Spec v2 is a WORKBENCH — sample's own structure: a settings pane plus a
...
ok((expanded.env.written.find(w => w.id === 'alltracks') || {}).html.indexOf('Song 8') > -1,
   '…and it expands IN PLACE (sample’s Generate fork answer, mirrored)');
...
// sample's constraint at GO: never paint fake sections. The waveform is the only
...
ok(/#blocks \{ display:grid; grid-template-columns:repeat\(3, minmax\(0,1fr\)\)/.test(css),
   'the grid is three per row (sample\u2019s spec), two per row under 1180px');

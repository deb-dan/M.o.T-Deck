"""THE GOLDEN-JOURNEY TEST — sample's whole user story, server-side, with no model.
...
WHAT A JOURNEY IN THIS FILE LOOKS LIKE, and every one of the five below follows it:
  ...
  3. ASSERT WHAT sample WOULD SEE: the card's before → after rows, the notes, the warnings.
...
#
# This is sample's transcript of 2026-08-28, op for op: the agent staged the amounts as
...
# ── what sample sees on the card, before she has pressed anything ──
...
# ⚠️ sample'S OWN RULING: "never a coercion the card didn't show". The row carries the
...
check("the range the SUM reads is entirely numeric — the fact the 0 came from",
      ...
      summed)
eq("…and it adds up to what sample asked for", sum(summed), WANT_TOTAL)
b.drop()
...
# the shape of sample's live Untitled.xlsx as openpyxl found it: every B cell a str, every
...
check("the CARD's sentence states the consequence and does NOT ask sample a question — "
...
check("…next to the remedy, which names BOTH outcomes and tells it not to ask sample",
      any("FIX IT NOW, IN THIS TURN" in n and "Do not ask sample" in n
          for n in w["notes"]))
...
#
# ⚠️ THIS IS THE ONE sample'S AMENDMENT 2 ADDED, AND IT IS THE ONE THAT MATTERS MOST: the
# aggregate-over-text condition must be repaired BY THE MODEL, inside its own turn, from
# the instruction in its tool result — not by a question put to sample and not by a button
...
# REPLACES the warned proposal (spec §1) — sample never sees the bad one.
healed = stage(b, [
...
# Stage a change; something else writes the workbook (the editor's writeback, sample's own
...
# files as ordinary workbooks — so sample could make one from the panel. The first apply on
...
wb = openpyxl.Workbook()
wb.active["A1"] = "MY REAL DATA"                 # a workbook sample actually has
wb.save(b.path("Report.pre-agent.xlsx"))
res = stage(b, [{"op": "set", "at": "A1", "values": [["changed"]]}], name="Report.xlsx")
got = apply(b, res)
eq("F-07 · sample's own Report.pre-agent.xlsx is UNTOUCHED by an apply on Report.xlsx",
...
# C4: and neither copy is listed as a workbook sample could open.
...
# data/office. sample has `Budget.xlsx` (with an undo stack) and `Budget.docx`. She renames
...
# bytes under the right name exercise the same path sample's real .docx would.
...
# ── F-27 · DATA sample HID CAME BACK ON SCREEN ─────────────────────────────────
...
eq("…and hd on the column sample hid, rather than 0 on every column",
...
# F-09: a column sample formatted `@` MEANS "this is text", and "1,200" was stored as the
...
#
# THE USER STORY: sample presses "Blank document" (or "Blank presentation"), the editor

...
# sample reads in the panel. Two different wordings of the same limit would be two
...
if HAVE_XL:
    ...
    # reference told sample to go and check formulas that could not possibly have moved —
...
# documents: a workbook sample actually had called `report.pre-agent.xlsx` was silently
...
# ⚠️ THE ESCAPE. A bare `stem + "-" + ext` would NOT be injective: a spreadsheet sample
...
if HAVE_XL:
    ...
    # an UNATTENDED agent write. In v2 the only caller left is sample's own Apply — and
...
if HAVE_XL:
    ...
    rc, _r = office_ops.apply_changeset(TMP, st["changeset_id"])
    time.sleep(0.05)
    make("fence.xlsx", [["sample typed this afterwards"]])       # the file moves
...
# ⚠️ THE MODEL IS THE FIRST LAYER OF THIS FIX, NOT THE BRIDGE'S INFERENCE (sample's ruling,
...
check("…and tells it to READ the result's warnings and fix them IN THE SAME TURN, which "
      "is what makes the SUM-over-text condition self-heal with sample never in the loop",
...
check("⚠️ AND IT IS TOLD NOT TO PUT THE TYPE DECISION TO sample. A dialog asking her "
      ...
      "Do not ask sample to decide a cell's type" in STAGE)
...
try:
    ...
    check("…and a junk ops list is a refusal WITH A SENTENCE, which the panel renders as "
          "a ✗ chip, so a model's bad call is visible to sample too",
          res["isError"] is True
...
# "Beta(2)"]: office._sheet_names' dedup step renamed sample'S Beta, and the card had only
...
if HAVE_XL:
    ...
    eq("F-16 · a rename onto a name another sheet holds is SKIPPED — sample's own sheet is "
...
# applied — sample reviews and applies this in LOffice" and their cards simply did not exist.
if HAVE_XL:
    office_ops.changeset_clear()
    _ids = []
...
# consent architecture caps what a steered model can WRITE (nothing, without sample's
...
# and a model that cannot see the cell cannot tell sample her spreadsheet contains one).
...
if HAVE_XL:
    ...
    check("office_read still returns the hostile cell VERBATIM — a fence never edits "
          "the document, because a model that cannot see the cell cannot warn sample "
          "it is there",

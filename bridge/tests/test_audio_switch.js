#!/usr/bin/env node
/* THE AUDIO-MODE SWITCH + THE BOUNDED COMPOSER — the fence (v1.5.26, Debi's design).
 *
 * WHY THIS FILE EXISTS. Two of Debi's rulings landed on the same twelve lines of the
...
console.log('2. the switch owns no state of its own');
{
  ...
  ok(!/localStorage/.test(SRC),
     ...
     + '(Debi\'s instruction, and the safe default — a hands-free mic that survives a '
...
console.log('3. the bounded composer, per design');
{
  ...
  ok(foc && /border:0/.test(foc.body),
     ...
     + 'this change removes)');
  // the order Debi asked for: textarea → mic → SEND → switch, all inside the field
...
const HIT = 44;
{
  ...
  ok(/flex-direction:column/.test(box.body), 'it is VERTICAL, as Debi drew it');
...
console.log('6. it is a radio group, semantically');
{
  ...
  for (const id of ['chat-auto', 'chat-audio-off', 'chat-conv']) {
    ...
    ok(new RegExp('id="' + id + '"[\\s\\S]{0,300}?title="').test(html),
       '…and a hover title, so the read survives for the pointer too (Debi\'s ask)');
  }

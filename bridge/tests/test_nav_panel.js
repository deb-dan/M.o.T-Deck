 ...
 *   • does a machine with NO saved layout render exactly today's sidebar? (sample's rule:
 ...
 *   • is the 13th pin refused with the limit named? (sample's ruling)
 *
...
console.log('registry');
{
  ...
  // sample's amendment: the two newest LANES are registry entries — that is precisely how
  ...
  // THE MUSIC PAIR — REWRITTEN AT THE CONSOLIDATION SLICE (sample 2026-08-29) AND NOT
  // WEAKENED. The Compose slice's rule was "both stay until sample picks"; she picked:
  ...
  // here it is sample's explicit ruling that both lanes coexist. `goose` is the PTY
...
console.log('defaults');
{
  ...
  // Logs — plus the components group. PHASE 2 adds exactly the two lanes sample asked
  ...
  // history and is sample's own ruling: `music` and `compose` were two rows for one job,
  ...
  // v1.5.26 — sample'S ORDER. Same eleven ids, new reading order (the deck · the three
  ...
  ok(top.join(',') === 'mc,hermes,unsloth,opencode,odysseus,voicestudio,comfyui,aider,loffice',
     'the default PINS are the nine, in sample\'s order: ' + top.join(','));
  ...
  ok(M.NAV_TOPBAR_PINS + M.NAV_TOPBAR_MRU === M.NAV_TOPBAR_MAX && M.NAV_TOPBAR_MAX === 12,
     '9 pins + a 3-slot window = sample\'s 12, unchanged');
...
console.log('validate');
{
  ...
  // sample's ruling, both sides of the boundary
  m = M.navNormalize({});
  ...
  ok(M.NAV_TOPBAR_MAX === 12, 'the STRIP is still sample\'s 12');
  ...
  // ⚠️ THE WINDOW ITSELF: nine pins, three slots, twelve tabs, whatever you open.
  {
    ...
    // naming it is refused (the vector sample hit: ⋯ click -> second Music tab).
...
// layout — the one thing sample's rule is actually about ("a fresh install looks like
...
// — so without a migration sample's reorder is invisible on sample's own Mac. The rule is
...
console.log('the reorder migration');
{
  ...
  ok(topIds(M.navMigrate(V1())) === NEW,
     'an UNTOUCHED v1 layout (byte-for-byte the old default) is replaced with sample\'s order');
...
// ── 7. the CUSTOMIZE overlay (v2 — sample rejected v1's static ↑ ↓ section) ──
//

...
def test_nav_registry_agrees_across_the_three_tables():
    ...
    assert len([1 for _i, p in nav.DEFAULT_TOPBAR if p]) <= nav.NAV_TOPBAR_MAX, (
        "the default layout must itself satisfy sample's 12-pin cap")
    flat = PANEL.replace(" ", "")
    ...
    # v1.5.26: the strip was REORDERED (sample), so this can no longer pin the pair of
...
# sample's report: everything running, and the tab shows "Nothing here yet — Create a
...
def test_the_workspace_is_made_a_real_git_project_at_install():
    ...
    shared 'global' project — which is exactly the empty Projects rail sample saw."""
...
# second one is what sample saw fail.
#
...
# catalog used to be rewritten only by its own Start, which is why sample's picker still
...
# days after sample deleted them in LM Studio.
...
def test_the_server_list_recovery_is_written_down():
    """sample removed 127.0.0.1:4096 from Settings -> Servers and could not re-add it.
...
#   + disabled_providers: ["llama.cpp"]  | NO        | NO      <- sample's exact symptom
...
# ══ "IT QUIETLY DEGRADED, WAS STILL SHOWING GREEN" (sample, 2026-08-21) ═════════
...
def test_the_feed_can_never_be_left_saying_lost_beside_a_green_card():
    ...
    sample's report: an episode that heals now SAYS it healed."""
...
def test_the_start_says_the_log_appends_so_n_blocks_is_n_starts():
    """sample saw the same two lines six times and read it as six servers. It is one

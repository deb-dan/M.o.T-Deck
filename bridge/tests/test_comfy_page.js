 ...
 * the new layout, sample's stage, and the real-data rule.
 *
 ...
 *
 *  4. ALL DESIGNS (sample's standing rule). Editorial + the three packs + studio
...
function runPage(state, gallery, mutate, store) {
  ...
  const fn = new Function(
    ...
    /* v3 — sample's stage + the hue type system, all pure and all executed below */
...
ok(/Free now: /.test(M.diskVerdict(STATE()).line),
   'the disk hover uses sample’s STATUS+DELTA grammar');
...
//     running. This is the screen sample looks at, and it must carry ZERO sentences.
...
//     sanctioned sentence (sample's ruling).
...
ok(/Open template in ComfyUI/.test(html),
   '…but it still exists (kept, demoted — sample’s fork answer 4)');
...
/* ⚠️ FIVE TRACKS, NOT THREE, SINCE sample'S REVIEW ("both side panes resizable"). The
...
ok(/--railw:208px/.test(rootBlk) && /--resw:224px/.test(rootBlk),
   'the rails DEFAULT to 208 / 224 px — generate-j’s measured 184 widened on sample’s '
...
// ── 7. sample'S STAGE — resizable and splittable, and both are real ───────────
...
// ── 8. THE REAL-DATA RULE (sample, at GO) ─────────────────────────────────────
...
// ── 9. THE DYNAMIC CATALOGUE ON THE PAGE (sample's review, 2026-08-29) ─────────
...
ok(staticNow.indexOf('id="promptbox"') < staticNow.indexOf('id="picker"')
   ...
   '…and it sits BETWEEN the settings/prompt region and Generate, which is where sample '
...
ok(pick.M.workflowVerdict(WF_SDXL, null).chip === 'ready'
   ...
   'the state chip is "ready" or the exact Get — sample’s "if it’s present/installed, '
   + 'it’ll work"');
...
ok(/clip_vision_h\.safetensors/.test(cam.markup)
   ...
   'the rail lists THIS workflow’s own files (sample’s bullet 2), including the 1.26 GB '
...
// ── 10. sample'S REVIEW: both rails resize, the results rail shows FOUR ────────
...
ok(/setVar\(\$\('frame'\), '--railw'/.test(html)
   ...
   + 'the rail — the thumbnails scale up because their columns do (sample: "the right '
   + 'rail AUTO-GROWS in correlation")');
ok(/max-height:calc\(2 \* var\(--thumb, 72px\) \+ 6px \+ 10px\)/
     .test(css.replace(/\s+/g, ' ')),
   'the results rail stands FOUR results (2×2) plus a 10px sliver — sample’s "4 media, '
...
/* ⚠️ THE DEFECT UNDER sample'S "6 SMALL, NOT 4": the peek was never the fallback by
...
ok(/#centre\.sized #lower \{ flex:1 1 auto; max-height:none/.test(css.replace(/\s+/g, ' ')),
   'dragging the stage SHORTER hands the freed height to the prompt (sample: "resize '
   + 'upward too — shrinking the stage frees bottom space that the layout uses")');

// ── 11. NO IN-APP DESTINATION ESCAPES TO THE BROWSER ────────────────────────
// sample's bug: the ⋯ menu's "ComfyUI tab ↗" opened Google Chrome instead of our own

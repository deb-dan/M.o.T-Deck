...
/* ⚠️⚠️ THE FOUR ASSERTIONS BELOW WERE REWRITTEN AT 2026-08-21k AND THAT IS A DELIBERATE
   ...
   formula is COPIED, never applied". sample asked for the opposite — "it seems not
...
// THE BUG, from sample's screenshot: "What does clean function do" → an explanation of
...
/* ══════════════════════════════════════════════════════════════════════════════
   ...
   THE MECHANISM IS NOT OPENAI FUNCTION-CALLING, ON PURPOSE: most of the models sample
...
eval(grab('agentGate'));
{
  ...
  /* ⚠️⚠️ THE THIRD VALUE IS THE WHOLE JUDGEMENT CALL OF THIS FUNCTION, AND IT IS PINNED
     ...
     lane on EVERY model sample owns. So null opens the lane with the requirement stated
...
// ⚠️ rewritten 2026-08-27 (was "preamble rides EVERY agent message"): sample's live
...
/* ⚠️ THIS BLOCK IS THE ONE THAT CHANGED MOST AT loffice-2026-08-28b, AND WHAT IT PINS IS
   THE INCIDENT'S ROOT CAUSE ON THE MODEL'S SIDE. sample's 2026-08-27 transcript shows the
...
eval(grab('agentBrief'));
{
  ...
  check('4. …and it says PLAINLY that the model cannot apply anything',
        ...
        && /Only sample can press Apply/.test(note));
...
}
{
  ...
  /* ⚠️⚠️ THE CARD'S JOB CHANGED COMPLETELY AT loffice-2026-08-28b AND SO DID THESE
     ...
     loud that it is not a spreadsheet change, because a card that let sample think she was
...
/* ⚠️⚠️ REWRITTEN AT loffice-2026-08-28d FOR TWO FINDINGS ON THE SAME FILE.
   ...
   a workbook sample genuinely had by that name was silently OVERWRITTEN by the first apply
   ...
   Plus the outcome line that reaches the model's next turn, and the ✗ chips. */
{
  ...
  //    highlight in v2 — this list IS the diff sample reads) ──
  const cs = {
...
/* ══ 6. THE ONE-EDITOR APPLY — WHERE A QUICK-LANE PLAN ACTUALLY GOES ═══════════
   (loffice-2026-08-28a, sample's one-editor ruling, roadmap §8)
   ...
   exact shape: a true-looking number over a write that did not happen. */
{
  ...
  eq('…with NO number-format op, because General is already right for a bare number and '
     + 'writing one would flatten a format sample had set herself',
...
/* ══ 7. TYPE HONESTY — THE 2026-08-28 INCIDENT, PINNED ON THE PAGE SIDE ════════
   ...
   WHAT HAPPENED, in one sentence: sample's budget was staged as the strings "$2,500",
   ...
   block is the page's half of all four:
     ...
     3. CONTEXT DECIDES THE REST, AUTOMATICALLY. No question is ever put to sample.
   ...
   on one lane and text on the other. If you change one, the other fails. */
{
  ...
  eq('a PLAIN number gets no pattern: General is already right, and writing one over it '
     + 'would flatten a cell sample had formatted herself',
  ...
  }
  {
    ...
    eq('…and the reason is recorded rather than turned into a question for sample',
  ...
  eq('nothing retyped draws nothing — a "0 values converted" line is noise',
     csTypeLines({}), []);
  /* ⚠️ NO QUESTION AND NO SECOND BUTTON (sample's ruling). The aggregate-over-text
     condition is the MODEL's to fix, inside its own turn, from the instruction in its
     tool result. If a stale changeset still carries it, sample is INFORMED. Apply stays
  ...
  check('…and when the dry-run had WARNED about that range, the line says that is what '
        + 'the 0 means rather than leaving sample to connect it',

...
`{id, pinned}` where **array order IS render order** (plus, since sample's 2026-08-29
...
NAV_ENTRIES = (
    ...
    # its surface is untouched; what changed at the music-consolidation slice (sample's
    ...
    # sample asked for ONE Music row and ONE Music tab: two rows for one job is the thing
    ...
    #
    # ⚠️ NO `tab_only` ANY MORE (v1.5.60, sample's live follow-up): tab_only let Classic
    ...
    # the Goose UI slice (sample's naming ruling 2026-08-29) because there are now two
    ...
    # music/compose pairs' argument a third time, and here it is sample's explicit ruling
    ...
    # you keep a tab on — and because the strip is at 11 of sample's 12 pins already, so
    ...
    # had just removed; sample saw it standing on her strip within the hour. This entry
    ...
    # draw it. sample asked for "an Unsloth-style API page, like Capabilities, only on
...
# it. The only additions are the two lanes sample asked for (aider, loffice), which had
...
#
# ⚠️ v1.5.26 — sample'S ORDER. The strip is now read left-to-right as "the deck, then the
...
#
# ⚠️ v1.5.49 — THE 9 + 3 STRIP (sample's ruling 2026-08-29). The strip is still at most
...
DEFAULT_TOPBAR = (
    ...
    # The strip is at 11 of NAV_TOPBAR_MAX=12 pins in sample's arranged order; a twelfth
    # would fill the last slot on a FRESH machine while doing nothing at all on sample's
    ...
    # sample's (her nav.json is stamped v2, so `migrate` returns early and `normalize`
    ...
    # declare a winner between two lanes sample's ruling says coexist. One sidebar click
    ...
    # seed, not pins. It joins the SWAPPABLE set, which is what sample's 9+3 ruling
...
# their old order, so a fresh machine's strip is unchanged and sample's own (whose saved
...
#
# `music` → `compose` is the music-consolidation slice (sample 2026-08-29): ONE Music door,
...
# have taken the Music tab off sample's strip and put a row labelled Music at the BOTTOM of
...
# there today and sample's rule for this slice is that a fresh install must look
...
# ⚠️ sample'S 12 IS UNCHANGED; WHAT THE 12 ARE MADE OF IS NOT (ruling 2026-08-29). The
...
NAV_TOPBAR_MAX = NAV_TOPBAR_PINS + NAV_TOPBAR_MRU   # 12 — sample's ruling, unchanged
...
def validate(model) -> str:
    ...
    Two rules, and both exist to stop a customisation from removing something:
      ...
      2. at most NAV_TOPBAR_PINS pinned tabs — the strip is a fixed-width centred
         ...
         sample's 2026-08-29 ruling the last three of those twelve are not pins at all but

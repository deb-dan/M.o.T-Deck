...
STARTER_CAP_BYTES = 14_000_000_000   # sample's locked S1 budget, DECIMAL GB (the unit
...
# ══ THE CURATION ═════════════════════════════════════════════════════════════
#
# HOW THIS SET WAS CHOSEN (sample's constraint: image AND video, ≤14 GB TOTAL, smallest
...
#
# ⚠️ WHY A CATALOG AT ALL (sample, 2026-08-29): "if one deletes models, it should be
...
# licence grant (spec §sample's locked answers), so it is refused whatever the disk says —
...
def model_key(row: dict, files: list) -> tuple:
    ...
    vocabulary sample uses — "the wan models, ltx, minimax h3". When the row has no model

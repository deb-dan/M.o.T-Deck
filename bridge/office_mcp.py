...
_OPS_ARG["description"] = (
        ...
        "Send every operation the request needs in ONE call: sample sees one card per "
        ...
        "computes 0 — silently, in a document sample will trust. So:\n"
        ...
        "result is listed on sample's card either way. That is a safety net for a mistake, "
    ...
    return [
        {
            ...
            "description": (
                ...
                "when it changed, whether sample has it open in LOffice right now and "
        ...
        },
        {
            ...
            "description": (
                ...
                "before and after, and hands sample ONE card in LOffice with the whole "
                ...
                "Do not ask sample to decide a cell's type. A proposal expires after "
...
def handle(root, message):
    ...
    if method == "initialize":
        return _res(rid, {
            ...
            "instructions": (
                "These tools read the spreadsheets in sample's Office folder and PROPOSE "
                ...
                "and sample applies it with a button in LOffice, which also keeps a "
...
def hermes_entry(port=8700) -> dict:
    ...
    EVERY KEY IS LOAD-BEARING:
      ...
      trust  — `untrusted` ARMS the approval gate for every write-capable tool on this
               ...
               write-capable, so this fires on nothing and sample sees no card — which is

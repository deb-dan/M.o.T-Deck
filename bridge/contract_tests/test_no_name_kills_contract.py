...
The rule, in sample's words: *agents/builders and MOT Deck may terminate ONLY
...
def test_hermes_ownership_has_no_name_signature():
    ...
    that string IS sample's standalone Hermes. Path evidence only."""
    code = _code(START)
...
def test_python_has_no_unowned_port_clear_helper():
    ...
    old MOT_DECK_PORT_TAKEOVER branch — the move that closed sample's Unsloth. No
...
def test_reap_pidfile_refuses_a_foreign_pid_and_reaps_our_own_live(tmp_path, monkeypatch):
    ...
    sample's standalone app, and the pid-recycling case), and one that IS ours must die."""
    ...
    #    ours — so a stand-in for sample's standalone app has to live somewhere else, as
    #    hers does.

...
def test_the_shim_source_defines_every_catalogued_method():
    ...
    # about the same call, and the catalogues are what the report quotes to sample.
    assert not (set(G.SHIMMED) & set(G.STUBBED))
    assert not (set(G.SHIMMED) & set(G.DEGRADED))
    assert not (set(G.STUBBED) & set(G.DEGRADED))
    # Every degradation is a SENTENCE, because the report owes sample one per lost feature.
...
def test_the_page_injects_the_shim_before_the_module_script():
    ...
    # …and OUR surface is named per sample's 2026-08-29 ruling, while nothing inside the
...
def test_no_pattern_or_port_scoped_kills_anywhere_in_this_lane():
    ...
    THE INCIDENT (example, not the rule): sample runs the standalone goose Desktop app on

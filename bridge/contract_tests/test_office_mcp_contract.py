...
THE SLICE. This file used to guard four MCP tools that WROTE sample's spreadsheets, by
...
mechanism to sample and that sentence must not become a lie. But they are no longer what
...
tells sample which mechanism is protecting her, and it must keep being true.
"""
...
def test_read_only_is_still_exactly_true_and_missing_annotations_fail_closed():
    ...
    assert "request_elicitation_consent" in src, (
        ...
        "write tools may no longer produce a card sample can see")

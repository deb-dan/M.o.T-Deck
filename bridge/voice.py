...
Verified argv surfaces (sample's Mac, 2026-08-07):
...
# absent (sample-observed on Qwen3-TTS-mlx), so "the same model" sounds like a
...
# (sample's A/B, then source-confirmed). Only the model's own config.json can tell
...
def voices_for_entry(entry: "dict | None") -> dict:
    ...
    Resolution order, strongest evidence first:
      ...
      3. config says base/voice_design (or declares an EMPTY spk_id) → no voices,
         plus an honest note. This is the case that fixes sample's Base model.
...
# WHY: OmniVoice renders a DIFFERENT random voice on every ▶ speak (sample-observed),
...
# WHY (sample's Mac, 2026-08-14): a 7.28 MB mp3 of ~30s+ was pinned successfully and then
...
def ref_stamp(path: object) -> str:
    ...
    This is what makes 'delete sample.wav, record a new sample.wav' a cache MISS. Without
...
# sample's ask: point the picker at folders that already hold usable clips (a
...
def resolve_render_voice(entry: dict) -> str:
    ...
    DEFAULT-VOICE FALLBACK (sample 2026-08-13: "model default" on the CustomVoice

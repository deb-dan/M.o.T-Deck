...
* **Source of truth:** `guards/harness-path-guard/` in the harness repo.
  `scripts/start_component.sh` seeds it to `~/.hermes/plugins/harness-path-guard/` on
  every Hermes start (copy-if-changed) and ensures `plugins.enabled` in
  `~/.hermes/config.yaml` contains `harness-path-guard`. **Never hand-edit the seeded
...
* **Mechanism:** the upstream `pre_tool_call` hook. `{"action":"approve"}` routes the
  ...
  Tier-2 dangerous commands), so the Harness panel renders its existing approval card
...
* **Policy:** `policy.yaml` (re-read on every gated call — edits apply without a
  ...
  `{HARNESS_ROOT}/data`, `/tmp`, `{TMPDIR}`. `deny` = `~/.ssh`, `~/.aws`, `~/.gnupg`,
...
* **Matching:** `os.path.commonpath([target, root]) == root` on realpath'd absolute
  paths. Never string prefixes (`/Users/debikEvil` must not match `/Users/debik`).

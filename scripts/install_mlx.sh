...
# Pins come from motdeck.yaml build.mlx_lm_pin / build.mlx_vlm_pin (single source of
...
# mlx-audio: the MLX half of MOT Deck-native voice capability. It already arrives
...
[[ -n "$MLX_LM_PIN" && -n "$MLX_VLM_PIN" ]] || { echo "ERROR: build.mlx_lm_pin / build.mlx_vlm_pin missing from motdeck.yaml"; exit 1; }
[[ -n "$MLX_AUDIO_PIN" ]] || { echo "ERROR: build.mlx_audio_pin missing from motdeck.yaml"; exit 1; }
[[ -n "$MLX_WHISPER_PIN" ]] || { echo "ERROR: build.mlx_whisper_pin missing from motdeck.yaml"; exit 1; }
uv pip install "mlx-lm==${MLX_LM_PIN}" "mlx-vlm==${MLX_VLM_PIN}" \
               "mlx-audio==${MLX_AUDIO_PIN}" "mlx-whisper==${MLX_WHISPER_PIN}"
deactivate

echo "[motdeck] mlx runtime installed:"

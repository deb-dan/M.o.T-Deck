...
_prepare_swift_config() {   # <Config.swift path> <harness root>
  local config_path="$1" harness_root="$2" tmp escaped
  escaped="$(_swift_string "$harness_root")"
  tmp="${config_path}.tmp.$$"
  {
    printf '%s\n' '// generated by ship.sh — do not edit'
    printf 'let harnessRoot = "%s"\n' "$escaped"
    printf '%s\n' 'let fatBuild = true'
...
_RESIGN=0
if [[ "$ROOT/app/main.swift" -nt "$APP/Contents/MacOS/Harness" || "$ROOT/app/Config.swift" -nt "$APP/Contents/MacOS/Harness" || "$config_changed" -eq 1 ]]; then

...
echo "[motdeck] Hermes tool-calling smoke test (this runs one real agent task; may take ~30-60s)..."
...
if echo "$OUT" | grep -q "HERMES_TOOLCALL_OK"; then
  echo "[motdeck] PASS — Hermes executed a tool call through the model endpoint. M0 Hermes criterion met."
else
  echo "[motdeck] FAIL — marker not found. Model may not tool-call reliably, or endpoint misconfigured."
  exit 1
fi

...
strip -x "$APP/Contents/MacOS/MOTDeck" 2>/dev/null || true

# ── ONE PUBLIC IDENTITY: MOT Deck (U150, 2026-09-07) ─────────────────────────
...
if [[ $FAT -eq 1 ]]; then
  ...
  if [[ -d "$OUT/.wheelhouse_cache" ]]; then
    echo "[motdeck] restoring $(ls "$OUT/.wheelhouse_cache"/*.whl 2>/dev/null | wc -l | tr -d ' ') wheels from $OUT/.wheelhouse_cache..."
    cp -n "$OUT/.wheelhouse_cache"/*.whl "$WHEELS/" 2>/dev/null || true
  fi
  ...
  # 4a.1 some projects pin a specific/older build backend (e.g. hermes pins setuptools==83.0.0).
  # Bundle both exact setuptools==83.0.0 and in-range setuptools<83 so PEP 517 build-isolation
  # envs resolve offline.
  "${DL[@]}" "setuptools==83.0.0" \
    || { echo "ERROR (fat): setuptools 83.0.0 download failed."; exit 1; }
  ...
  mkdir -p "$OUT/.wheelhouse_cache"
  cp -n "$WHEELS"/*.whl "$OUT/.wheelhouse_cache/" 2>/dev/null || true
  ...
  codesign --force --deep --sign - "$APP" \
    && echo "[motdeck] ad-hoc signed (sample's 2nd Mac may still need:  xattr -dr com.apple.quarantine <installed-app-path>)" \

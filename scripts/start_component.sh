...
# collides: sample's STANDALONE Unsloth app listens on :8888 and our own start was
...
# identity, and sample runs standalone copies of the very apps we embed.
...
PYRESOLVE
    ...
    # SECOND source — keys sample mints in MOT Deck → API — in one ephemeral 0600 launch
    ...
    #
    # v2 (2026-08-20, sample ruling): the two numbers below are now the FALLBACK — a
...
PYWIRE
  ...
  comfyui)
    ...
    #                 instant crash on a fresh base dir. This is the crash sample hit.
    ...
    #                 tree sample is told to drop checkpoints into actually exists.
  ...
  unsloth)
    ...
    #
    # ISOLATED HOME (2026-08-28). Upstream's default home is ~/.unsloth, which sample's
    ...
    # the port sample's STANDALONE Unsloth app listens on (the port clear below would kill it).
  ...
  deepseek)
    ...
    # bare `python3` (sample's is /Users/testuser/.local/bin/python3) does NOT have it — so
...
PYGUARD
    ...
    # sample added by hand survive the whole-map load/store.
    #
...
PYLOFFICE
    ...
    # sample runs a STANDALONE Hermes out of ~/.hermes/hermes-agent/venv. Both lines
